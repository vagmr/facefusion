...
def get_temp_directory_path(target_path: str) -> str:
    ...
    temp_directory_path = os.path.join("", "facefusion")
...
def list_directory(directory_path: str) -> Optional[List[str]]:
    if is_directory(directory_path):
        ...
        files = [
            Path(file).stem
            for file in files
            if not Path(file).stem.startswith((".", "__"))
        ]
        return sorted(files)
    return None
...
def sanitize_path_for_windows(full_path: str) -> Optional[str]:
    ...
    while True:
        ...
        buffer_threshold = ctypes.windll.kernel32.GetShortPathNameW(
            full_path, unicode_buffer, buffer_size
